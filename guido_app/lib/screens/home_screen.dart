...
class HomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Course? featured = courses.isEmpty
        ? null
        : (courses.any((c) => c.isEnrolled)
            ? courses.firstWhere((c) => c.isEnrolled)
            : courses.first);

    final moreCourses =
        courses.where((c) => featured == null || c.slug != featured.slug).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => CourseDetailScreen(slug: c.slug),
        ),
      ),
      child: Container(
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 10,
              decoration: BoxDecoration(
                color: _difficultyDotColor(c.difficulty),
                shape: BoxShape.circle,
              ),
            ),
            const SizedBox(width: 12),
