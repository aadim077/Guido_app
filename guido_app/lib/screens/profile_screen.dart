  ...
  Widget build(BuildContext context) {
    return Consumer<ReminderProvider>(
      builder: (context, provider, _) {
        final statusText = provider.isReminderEnabled
            ? 'Reminder set for ${_formatTime(provider.selectedTime)}'
            : 'No reminder set';
        final statusColor = provider.isReminderEnabled ? const Color(0xFF16A34A) : const Color(0xFF9CA3AF);

        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Settings', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
            const SizedBox(height: 12),
            Container(
              decoration: BoxDecoration(
                color: Colors.white,
              ...
              child: ListTile(
                ...
                title: const Text('Notifications & Preferences', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                subtitle: Row(
                  children: [
                    Container(width: 8, height: 8, decoration: BoxDecoration(color: statusColor, shape: BoxShape.circle)),
                    const SizedBox(width: 6),
                    Text(statusText, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
                  ],
                ),
